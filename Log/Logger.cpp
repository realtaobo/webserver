...
const string& Logger::getLogFileName() { return logFileName_; }
...
void Logger::Impl::formatTime()  // muduo
{
  ...
  struct tm* p_time = localtime(
      &time);  // http://www.cplusplus.com/reference/ctime/localtime/?kw=localtime
  strftime(str_t, 26, "%Y-%m-%d %H:%M:%S\n", p_time);  //<time.h>
  stream_ << str_t;
}

Logger::Logger(const string& fileName, int line) : impl_(fileName, line) {}
